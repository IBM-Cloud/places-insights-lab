...
server.method('api_tweets',
  function (query, next) {
    ...
    twitter.search(query + " AND posted:" + fewDaysAgo, 10, function (error, body) {
      next(error, body.tweets);
    });
  }, {
    cache: {
      expiresIn: 60 * 60 * 1000, // caching duration
      generateTimeout: 30 * 1000 // timeout to get the first request
    }
  }
);
...
server.route({
  ...
  handler: function (request, reply) {
    server.methods.api_tweets(request.query.query, function (err, result) {
      reply(err, result);
    });
  }
});
